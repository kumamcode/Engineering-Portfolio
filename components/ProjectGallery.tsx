...
export default function ProjectGallery({ projects }: ProjectGalleryProps) {
  ...
  const stockBotProject = useMemo(() => {
    if (selectedCategory === 'Software/AI') {
      return filteredProjects.find(p => p.id === '12');
    }
    return null;
  ...
  const otherProjects = useMemo(() => {
    if (selectedCategory === 'Software/AI') {
      return filteredProjects.filter(p => p.id !== '12');
    }
    return filteredProjects;
  }, [filteredProjects, selectedCategory]);

  // Prepare all images for lightbox
  const prepareLightboxImages = (projectIndex: number, imageIndex: number) => {
    ...
    for (let i = 0; i < projectIndex; i++) {
      flatIndex += filteredProjects[i].images.length;
    }
  ...
  const renderProjectCard = (project: Project, projectIndex: number, projectsList: Project[]) => {
      ...
      >
        ...
        {project.images[0] && (
          <div
            ...
            onClick={() => prepareLightboxImages(actualIndex, 0)}
          >
            <Image
  ...
  return (
    <div className="w-full">
      ...
      {/* Special 2-column layout for Stock Bot when Software/AI is selected */}
      {stockBotProject && (
      ...
      {otherProjects.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {otherProjects.map((project, index) => renderProjectCard(project, index, otherProjects))}
        </div>
      )}
